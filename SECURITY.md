...
- **GitHub codeload 的 per-entry PAX 仅有一个窄例外。** 只有来源已被解析并固定到 GitHub 的 40 位 commit、该条目携带 global 头且其 `comment` 逐字等于该 pin、且该条目有效 PAX 键集合 ⊆ 显式白名单 `{atime, charset, comment, ctime, dev, gid, gname, ino, linkpath, mode, mtime, nlink, path, size, uid, uname}` 时才允许。白名单键或为部署语义忽略项（时间/属主/mode/设备号/charset——live codeload 的 per-entry `comment` 实测为空，故不作 per-entry pin），或随非普通条目跳过（linkpath），或下游完整复验（`path` 成为最终路径后仍过 `safeEntryPath` 遍历/绝对路径/非法段/碰撞校验；`size` 若撒谎会使 strict parser 失步报错）。provenance 钉在 **global 头** comment==commit，规则未放宽。白名单外任何键、缺 global pin、错误 pin、非 GitHub 归档、未消费 metadata 与任何不安全 global header 都保持 `ARCHIVE_UNSAFE`；install 与 compose 走同一预检。
